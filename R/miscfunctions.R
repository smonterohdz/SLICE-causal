# Date			    Element			    Change
# Jun 05 2019   synt.mdl()      verified for zero degree nodes and 
#                               zero undirected edges in the equivalence class
#                               returning the CPDAG
# May 29 2019 	disturb() 		  previous parameters(type, p, nsamp, p1, p2)
# Apr 30 2019 	induced.graph() added
# Nov 28 2018 	commented the use of letter as nodes names synt.mdl <- function()
# Sep 01 2018 	add.alpha function added
#============================================================================== 
## Generates a synthetic model with 'p' variables
synt.mdl <- function(p, pconn){
  zerodegFlag <- TRUE
  zeroundirFlag <- TRUE
  while (zerodegFlag | zeroundirFlag ) {
    #Uncomment for using letters as nodes' names
    V.names <-paste0("V", 1:p)
    myDAG <- pcalg::randomDAG(p, prob=pconn, V=V.names) 
    am <- (as(myDAG,"matrix")!=0)*1L #In case a weighted graph
    ...
    if (any(colSums(am + t(am)) == 0)) {
      ...
      am <- as(trueCPDAG,"matrix")
      undir.res <- get.undiredges(am)
      zeroundirFlag <- undir.res$zeroundirFlag  
    }
  }
  model <- list(mdl=myDAG,cpdag=trueCPDAG)
  return(model)
}
...
#============================================================================== 
## Generates a vector with Gaussian and non-Gaussian error terms
# parameter 'a' deviates
disturb <- function(type, p, nsamp, p1, p2, a = 0){
  p2.norm <- runif(n = p, min = p2-a, max = p2+a)
  errMat <- switch(type,
    normal = matrix(rnorm(nsamp * p, mean = rep(p1,p),sd = (p2.norm)**2), nrow = nsamp,byrow = TRUE),
	...
	subsupergaussian = genErrDist(p = p,nsamp = nsamp,errminmax =c(p1, p2.norm))
  )
  return(errMat)
}
...
mycomparegraphs3 <- function(mdl.true,mdl.est,mdl.cpdag){
  ...
  shd.val <- pcalg::shd(mdl.true,mdl.est)
...
orientation <- function(df, beta, cpdag.struct) {
  ...
  if(!is.null(edgeToadd)){
    ...
    nodes.lab <- cpdag.struct@nodes
    cpdag.struct.pdag <-addBgKnowledge(getGraph(cpdag.struct), nodes.lab[edgeToadd[, 1]], nodes.lab[edgeToadd[, 2]])
    if (is.null(cpdag.struct.pdag) == TRUE) {
      #warning("Meek rules could not be added. Trying  adding only one edge.")
      cpdag.struct.pdag <-
        removeEdge(as.character(nodes.lab[edgeToadd[,2]]),
                   as.character(nodes.lab[edgeToadd[,1]]),
#                   getGraph(cpdag.struct))
      ...
      cpdag.struct.pdag <-
        addEdge(as.character(nodes.lab[edgeToadd[,1]]),
                as.character(nodes.lab[edgeToadd[,2]]),
...
#==============================================================================
## Extracts induced graphs in 'mdl.true' and 'mdl.est' from the 
## subgrpah consisting of undirected edges in 'mdl.cpdag'
induced.graph <- function(mdl.true, mdl.est, mdl.cpdag){
    ...
    return(list(mdl.true=mdl.true, mdl.est=mdl.est))
}
