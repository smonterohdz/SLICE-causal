...
#' represented bya a completed partially directed acyclic graphCPDAG
#' 
...
slicegm.oracle <- function(G.am, V.data, ceff.mat) {
  ...
  while (nr.eUndir > 0 & cpdagFlag == FALSE) {
    ...
    for (i in 1:nr.eUndir) {
      ...
      ce.xy <- abs(ceff.mat[x,y])
      ce.yx <- abs(ceff.mat[y,x])
